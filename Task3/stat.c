...
void mean_variance(const int v[], int n, double* mean, double* variance) 
{
	...
	*mean = round(sum / (double)n * 100.0) / 100.0;
	
	double var = 0.0;
	for (int i = 0; i < n; i++)
		var += pow(v[i]-*mean, 2);

	*variance = round(var / (double)n * 100.0) / 100.0;
}
...
{
	for (int i = 0; i < n; i++)
		v[i] = (rand() < probability * RAND_MAX) ? 1 : 0;
}
...
{
	for (int i = x_start - 2; i < 11; i++)
	{
		if (i < 8) printf(" %i", i + 2);
		else		printf("%i", i + 2);

		printf(" |");
		int yVal = round(v[i] / y_scale);
...
void monty_hall(int n, int* p_switch_wins) 
{
	*p_switch_wins = 0;
	for (int i = 0; i < n; i++)
	{
		int winPos = rand() % 3;
		int choice = rand() % 3;
		if (choice != winPos) (*p_switch_wins)++;
	}
}
